...
class Network:
    ...
    def residual_scalars(self) -> list[float]:
        ...
        for comp in self.component_list:
            ...
            scalars = comp.residual_scalar

            if len(residuals) != len(scalars):
                mismatches.append(
                    f"{comp.name} ({comp.__class__.__name__}): "
                    f"{len(residuals)} residuals, "
                    f"{len(scalars)} residual scalars"
                )

            for i, scalar in enumerate(scalars):
                if scalar == 0.0:
                    zero_scalars.append(
                        f"{comp.name} ({comp.__class__.__name__}) scalar[{i}]"
                    )

        # balances
        for bal in self.balance_list:

            residuals = bal.residuals
            scalars = bal.residual_scalar

            if len(residuals) != len(scalars):
                mismatches.append(
                    f"{bal.name} ({bal.__class__.__name__}): "
                    f"{len(residuals)} residuals, "
                    f"{len(scalars)} residual scalars"
                )
            ...
            for i, scalar in enumerate(scalars):
                if scalar == 0.0:
                    zero_scalars.append(
                        f"{bal.name} ({bal.__class__.__name__}) scalar[{i}]"
                    )

        if mismatches or zero_scalars:

            lines = ["Residual scalar validation failed."]

            if mismatches:
                lines.extend([
                    "",
                    "Each component/balance must provide exactly",
                    "one residual scalar per residual.",
                    "",
                    "Mismatched objects:",
            ...
            if zero_scalars:
                lines.extend([
                    "",
                    "Residual scalars cannot be zero.",
                    "",
                    "Zero residual scalars:",
                    *[f"  - {z}" for z in zero_scalars],
                ])
        ...
        return (
            [
                scalar
                for comp in self.component_list
                for scalar in comp.residual_scalar
            ]
            +
            [
                scalar
                for bal in self.balance_list
                for scalar in bal.residual_scalar
            ]
        )


    @property
    def scaled_residuals(self) -> list[float]:

        return [
            residual / scalar
            for residual, scalar in zip(
                self.residuals,
                self.residual_scalars,
            )
        ]
